import { useState } from "react";

export default function SwitchButton({ func, job }) {
  const [changebutton, setbutton] = useState(true);

  const changeSt = () => {
    setbutton(!changebutton);
    func(!job);
  };

  return (
    <div className=" mt-5 ml-20 flex ">
      {!changebutton ? (
        <button
          type="button"
          className="text-white bg-gradient-to-br from-purple-600 to-blue-500 hover:bg-gradient-to-bl  focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 mb-2"
          onClick={changeSt}
        >
          Close
        </button>
      ) : (
        <button
          type="button"
          className="mg-t text-white bg-gradient-to-br from-purple-600 to-blue-500 hover:bg-gradient-to-bl  focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 mb-2"
          onClick={changeSt}
        >
          Create Job
        </button>
      )}
    </div>
  );
}
